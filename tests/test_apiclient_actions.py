...
class TestActionManager(unittest.TestCase):
    ...
    def test_create_fail_when_api_return_error_code(self, mock_requests):
        ...
        mock_requests.post.return_value = self.mock_response
        self.assertRaises(exceptions.MetadataCreationFailure, self.action_manager.create, {'action': 'metadata'})
    ...
    def test_delete_fail(self, mock_requests):
        self.mock_response.status_code = 500
        mock_requests.delete.return_value = self.mock_response
        self.assertRaises(exceptions.MetadataDeleteFailure, self.action_manager.delete, 'test_action_id')
    ...
    def test_get_fails_on_error_different_from_404(self, mock_requests):
        self.mock_response.status_code = 500
        mock_requests.get.return_value = self.mock_response
        self.assertRaises(exceptions.MetadataGetFailure, self.action_manager.get, 'test_action_id')
    ...
    def test_list_error(self, mock_requests):
        ...
        mock_requests.get.return_value = self.mock_response
        self.assertRaises(exceptions.MetadataGetFailure, self.action_manager.list)
    ...
    def test_update_raise_MetadataUpdateFailure_when_api_return_error_code(self, mock_requests):
        ...
        mock_requests.patch.return_value = self.mock_response
        self.assertRaises(exceptions.MetadataUpdateFailure, self.action_manager.update,
                          'd454beec-1f3c-4d11-aa1a-404116a40502', {'status': 'bamboozled'})


    @patch('freezer.apiclient.actions.requests')
    def test_set_status_calls_update_with_correct_update_doc(self, mock_requests):
        self.mock_response.status_code = 200
        self.mock_response.json.return_value = {
            "patch": {"status": "bamboozled"},
            "version": 12,
        ...
        mock_requests.patch.return_value = self.mock_response
        retval = self.action_manager.set_status('d454beec-1f3c-4d11-aa1a-404116a40502', {'status': 'iced'})
        self.assertEqual(retval, 12)
        endpoint = 'http://testendpoint:9999/v1/actions/d454beec-1f3c-4d11-aa1a-404116a40502'
        headers = {'X-Auth-Token': 'testtoken'}
        mock_requests.patch.assert_called_with(endpoint, headers=headers, data='{"status": {"status": "iced"}}')
